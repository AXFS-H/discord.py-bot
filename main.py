...
async def on_message(message):
    ...
    if message.content.startswith('%sricardo' % prefix):
        req = requests.get(
            'https://api.tenor.com/v1/search?media_filter=minimal&key=%s&q=%s&limit=%s' % (api_key, search, limit))
        if req.status_code == 200:
            results = json.loads(req.content)["results"]
            choice = random.choice(results)
            file_url = str(choice.get("media")[0].get("tinygif").get("url"))
            embed = discord.Embed(title="Ricardo is dancing :man_dancing: ", color=0xe63e58).set_footer(
                text="Ricardo Milos", icon_url="https://i.imgur.com/VJ8oioL.jpg").set_image(url=file_url)
